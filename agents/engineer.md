...
The folder must be: `agent-logs/YYYY-MM-DD-HH-MM`

Where `YYYY-MM-DD-HH-MM` is the exact string returned by that command. The task log is `task-name.md` inside that folder, where `task-name` is a short kebab-case description of the task. Screenshots go in the same folder.
